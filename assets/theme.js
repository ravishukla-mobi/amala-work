...
(function (factory) {
  ...
  var ExitPopup = /*#__PURE__*/ (function () {
    function ExitPopup(element) {
      ...
      var subscriptionBanner = document.querySelector(
        '[data-section-type="subscription_banner"]'
      );

      if(theme.pageType == 'collection' || theme.pageType == 'product' || theme.pageType == 'index' || subscriptionBanner.length != 0){
        let _this = this;
        let bannerClass = document.querySelector('.collection_subscription_topbanner');
        bannerClass.addEventListener("click", function () {
          _this._openPopup();
        });
        let closebannerClass = document.querySelector('.exit-popup [data-action="close-popup"]');
        closebannerClass.addEventListener("click", function () {
          _this.element.setAttribute("aria-hidden", "true");
          window.theme.isExitPopupOpen = false;
        });
      }

      if(theme.pageType == 'product' || subscriptionBanner.length != 0){
        let _this = this;
        let bannerClass = document.querySelector('.collection_subscription_topbanner_mindful');
        bannerClass.addEventListener("click", function () {
          _this._openPopup();
        });
        let closebannerClass = document.querySelector('.exit-popup [data-action="close-popup"]');
  ...
  var AppPopupSection = /*#__PURE__*/ (function () {
    function AppPopupSection(element) {
      this.element = element;
      this.delegateElement = new Delegate(this.element);
      this.rootDelegateElement = new Delegate(document.body);
      this.hasOpenOnceInCurrentPage = false; // If the popup has been already displayed, we do not display it
      ...
      if(iOS()){
        let ios_url = this.element.querySelector('a').getAttribute('data-apple');
        this.element.querySelector('a').setAttribute("href", ios_url);
      }else{
        let android_url = this.element.querySelector('a').getAttribute('data-android');
        this.element.querySelector('a').setAttribute("href", android_url);
      }
    ...
    _createClass(AppPopupSection, [
      ...
      },
      {
        ...
        value: function _attachListeners() {
          ...
          this.delegateElement.on(
            ...
            if (localStorage.getItem("themeAppPopup") === 'false'){
              setTimeout(function(){
                _this._openPopup();
              },5000);
            }else{
              if(theme.pageType == 'collection'){
                if (localStorage.getItem("themeAppPopupCol") != 'true'){
                  setTimeout(function(){
                    _this._openPopup();
                  },5000);
                  localStorage.setItem("themeAppPopupCol", "true");
                }
              }else{
                if (localStorage.getItem("themeAppPopup") != 'true'){
                  localStorage.setItem("themeAppPopup", "false");
                }
              }
            }
      ...
      },
      {
        ...
        value: function _openPopup() {
          if (!window.theme.isNewsletterPopupOpen) {
            this.element.setAttribute("aria-hidden", "false");
            localStorage.setItem("themeAppPopup", "true");
            this.hasOpenOnceInCurrentPage = true;
            window.theme.isAppPopupOpen = true;
            this.delegateElement.on("click", this._clickOutsideListener);
            this.rootDelegateElement.on("keyup", this._onKeyPressedListener);
          }
        },
      },
      {
        ...
        value: function _closePopup() {
          this.element.setAttribute("aria-hidden", "true");
          window.theme.isAppPopupOpen = false;
          this.delegateElement.off("click", this._clickOutsideListener);
          this.rootDelegateElement.off("keyup", this._onKeyPressedListener);
        },
      },
  ...
  var Cart = /*#__PURE__*/ (function () {
    ...
    _createClass(Cart, [
        ...
        value: function _attachListeners() {
          if (
            ...
            if (
              ...
              Responsive.getCurrentBreakpoint() !== "tablet"
            ) {
              this.delegateElement.on(
                "click",
                ".mc_estimate_delivery_btn",
                this._onETAClick.bind(this)
              );
            }
          }
          ...
          if (this._getCookie("lastUserPinCode") != "" && this._getCookie("lastUserPinCode") != "undefined") {
            $("#mc_pin_code_to").val(this._getCookie("lastUserPinCode"));
            $(".mc_estimate_delivery_btn").trigger("click");
          }

          let bundletotalInterval = setInterval(function () {
            let bundleTotal = $('.th_cart_total_sale_price');
            if(bundleTotal.length != 0){
              let comparePrice= 0,finalPrice = 0,shippingPrice = parseInt($('.cart-recap__shipping-price').attr('data-shipping')),shippingthreshold = parseInt($('.cart-recap__shipping-price').attr('data-threshold'));
              $('.th_pb_cart_line').each(function(){
                comparePrice = comparePrice + parseInt($(this).attr('data-thpriceoriginal'));
                finalPrice = finalPrice + parseInt($(this).attr('data-thprice'));
              });
              console.log(finalPrice,shippingthreshold);
              if(finalPrice <= shippingthreshold){
                comparePrice = comparePrice + shippingPrice;
                finalPrice = finalPrice + shippingPrice;
              }
              $('.th_cart_total_sale_price').html(Currency.formatMoney(finalPrice,window.theme.moneyFormat));
              $('.th_cart_total_original_price').html(Currency.formatMoney(comparePrice,window.theme.moneyFormat));
              clearInterval(bundletotalInterval);
            }
          }, 100);
          setTimeout(function(){clearInterval(bundletotalInterval)},3000);
        },
      },
      {
        key: "_getCookie",
      ...
      {
        key: "_onETAClick",
        value: function _onETAClick(event) {
          if (event) {
            event.preventDefault();
          }
          var mc_AED_loader = $("#mc_estimate_delivery .loader_span");
          var mc_AED_error = $("#mc_estimated_delivery_error_msg");
          var mc_AED_input = $("#mc_estimate_delivery #mc_pin_code_to");
          var mc_AED_button = $(".mc_estimate_delivery_btn");
          var mc_AED_button_close = $("#mc_estimate_delivery .cross-submit");
          var mc_AED_cart_items = $(".mini-cart__line-item")
            .map(function () {
              return this;
            })
            .get();
          mc_AED_button.addClass("hide");
          $(".mc_estimate_delivery_msg").removeClass("animate");
          ...
          mc_AED_button.addClass("hide");
          mc_AED_loader.removeClass("hide");
          mc_AED_error.html("");
          if (mc_AED_input.val() == "") {
            mc_AED_error.html("Please enter your pincode.");
            mc_AED_button.removeClass("hide");
            mc_AED_loader.addClass("hide");
          } else {
            mc_AED_button.addClass("hide");
            mc_AED_loader.removeClass("hide");
            mc_AED_error.html("");

            var zipRegex = /^[1-9]{1}[0-9]{2}\s{0,1}[0-9]{3}$/;
            var pin_code_to = mc_AED_input.val();

            if (!zipRegex.test(pin_code_to)) {
              mc_AED_error.html("Invalid pincode.");
              mc_AED_button.removeClass("hide");
              mc_AED_loader.addClass("hide");
            } else {
              var i = 1;
              setCookie("lastUserPinCode", mc_AED_input.val());
              setTimeout(function () {
                var currentItem = 0;
                function do_ajax() {
                  ...
                  if (currentItem < $(".cart_line-item").length) {
                    currentItem++;
                    var $this = $('#cart-line-item-'+currentItem),
                        packing_time = parseInt($this.find('p#mc_packing_time').attr('data-packing-time')),
                        pickup_postcode = $this.find(".product_meta_pincode").html().trim(),
                        delivery_postcode = mc_AED_input.val(),
                        sku_code = $this.attr('data-variant-id'),
                        weight = $this.attr('data-item-weight');
                    ...
                    $.ajax({
                      ...
                      success: function (response) {
                        if(response.status == true){
                          $this.siblings(".estimated_delivery_message").addClass("heighted");
                          
                          let estimated_delivery_date = addPackagePrepDays(response.estimated_delivery_date, 0);
                          
                          $this.siblings(".estimated_delivery_message").find("#mc_estimate_delivery_msg-"+i).html("Estimated Delivery by " + estimated_delivery_date).addClass("animate");
                          if (mc_AED_error.html() == "") {
                            $this.find(".mc_estimate_delivery_msg").removeClass("hide_delivery");
                          }
                          i = i + 1;
                          do_ajax();
                          if (currentItem == $(".cart_line-item").length) {
                          ...
                          mc_AED_loader.addClass("hide");
                          mc_AED_button.removeClass("hide");
                          mc_AED_error.html(response.message);
                          $(".mc_estimate_delivery_msg").removeClass("animate").addClass("hide_delivery");
                          $('.estimated_delivery_message').removeClass('heighted');
                          mc_AED_button_close.addClass('hide_cross');
                          mc_AED_button.removeClass("hide");
                          mc_AED_loader.addClass("hide");
                        }
                      },
        ...
        value: function _rerender() {
          ...
          if (showToast == true) {
            ...
            setTimeout(function () {
              ...
              if(localStorage.getItem("mainproductaddtocart")=="showrelevant"){
                openRelatedProductPopup();
                localStorage.setItem("mainproductaddtocart", "");
              }
            ...
            content.text().then(function (html) {
              ...
              if (window.theme.cartType !== "page") {
                if (window.theme.pageType !== "cart") {
                  ...
                  _this2.flickityInstance = new js(
                    originalCart.querySelector(".upsell_products_slider"),
                    {
    ...
    _createClass(ModalManager, [
      ...
      },
      {
        ...
        value: function _openModal(event, target) {
          ...
          if (!event.target.classList.contains("modal_address_btn")) {
            document
              .getElementById(target.getAttribute("aria-controls"))
              .querySelector(".modal__inner").innerHTML = "";
          }
          if (modal) {
            var onOpen = function onOpen(event) {
              if (event.propertyName === "visibility") {
                ...
                // if (window.theme.pageType == "search" || window.theme.pageType == "product") {
                  _this._openQuickView(target);
                // }
              }
            };
      ...
      {
        key: "_openQuickView",
        value: function _openQuickView(target) {
          var modal = document.getElementById(
            target.getAttribute("aria-controls")
          );
          modal.classList.add("is-loading");
          if(window.theme.pageType == "product" || window.theme.pageType == "cart" || window.theme.pageType == 'customers/order') {
      ...
      },
      {
        ...
        value: function _checkClickOutside(event) {
          if (
            ...
            var container = $(".pop-up-modal.size-chart__popup-modal.show-pop-up");
            var container2 = $(".cus_close_btn");
        ...
        value: function _renderAvailabilitySection(id) {
          ...
          return fetch(
            ...
            return response.text().then(function (content) {
              ...
              if(availabilityModal != null) document.body.appendChild(availabilityModal);
            });
          });
        },
      },
  ...
  var ProductVariants = /*#__PURE__*/ (function () {
    function ProductVariants(element, options) {
      ...
      this._attachListeners();
      if(localStorage.getItem('currentVariantTitle') != '' && localStorage.getItem('currentVariantTitle') != null && localStorage.getItem('currentVariantTitle') != undefined){
        const currentVariantTitle = localStorage.getItem('currentVariantTitle');
        let selectedOption = document.querySelector(`.block-swatch__radio[value="${currentVariantTitle}"]`);
        if(selectedOption != null){
          $(`.block-swatch__radio[value="${currentVariantTitle}"]`).prop('checked',true);
          setTimeout(function(){_this._onOptionChanged('',selectedOption);},1000);
        }
      }
    }
    ...
    _createClass(ProductVariants, [
      ...
      },
      {
        ...
        value: function _onVariantChanged(previousVariant, newVariant) {
          ...
          this._updateWeight(newVariant, previousVariant); // Update weight for get delivery date
      ...
      },
      {
        ...
        value: function _updateProductPrices(newVariant, previousVariant) {
          var productPricesAll = this.element.querySelectorAll(
            ".price-list.actual-product-price"
          );

          $(productPricesAll).each(function (index, productPrices) {
            if (!productPrices) {
            ...
            if (!newVariant) {
              ...
              if (newVariant["compare_at_price"] > newVariant["price"]) {
                ...
                if (
                  $(productPrices).parents(".modal").length < 1 &&
                  $(productPrices).parents("form.product-form").length > 0
                ) {
                } else {
                  productPrices.innerHTML +=
              ...
              var qty = parseInt($(this).closest('.card__section').find('[name="quantity"]').val());
              $(this).closest('.card__section').find('.sticky-on-mobile .price--highlight').attr('data-price',newVariant["price"]).html(Currency.formatMoney(newVariant["price"] * qty, window.theme.moneyFormat));
              $(this).closest('.card__section').find('button.product-form__add-button .price--highlight').attr('data-price',newVariant["price"]).html(Currency.formatMoney(newVariant["price"] * qty, window.theme.moneyFormat));
            }
          });


          var productPricesListAll = this.element.querySelectorAll(
            ".dynamic_quantity_actual_price"
          );
          $(productPricesListAll).each(function (index, productListPrices) {
            ...
            if (!newVariant) {
              ...
              productListPrices.innerHTML = Currency.formatMoney(
                        newVariant["price"],
                        window.theme.moneyFormat
                      );
            }
      ...
      },
      {
        ...
        value: function _updateSku(newVariant, previousVariant) {
          var productSku = this.element.querySelector(".product-meta__sku"),
              detailProductSku = document.querySelector('.detail-product-meta__sku-number');

          if (!productSku && !detailProductSku) {
            return;
          }
          ...
          if (!newVariant || newVariant["sku"] === "") {
            ...
            if (detailProductSku) {
              detailProductSku.innerHTML = newVariant["sku"];
            }
          }
        },
        /**
      ...
      },
      {
        ...
        value: function _updateDiscountLabel(newVariant, previousVariant) {
          ...
          var discountLabel = this.element.querySelector(
            ".product-meta .product-label--p-sale"
          );
          var discountAmountLabel = this.element.querySelector(
          ...
          if (
            ...
            if (
              $(discountLabel).parents(".modal").length < 1 &&
              savings != "0%"
            ) {
              discountLabel.innerHTML = "" + savings + " OFF";
            } else {
              discountLabel.innerHTML = "Save " + savings + "";
            }

            if (discountAmountLabel != null) {
              discountAmountLabel.innerHTML = "You Save: " + savingsAmount;
              discountAmountLabel.style.display = "inline-block";
            }
      ...
      },
      {
        ...
        value: function _updateAddToCartButton(newVariant) {
          ...
          if (!newVariant) {
            ...
            var addToCartContent = addToCartButtonElement.innerHTML;
             addToCartButtonElement.innerHTML = addToCartContent.replace('Add to cart', window.languages.productFormUnavailable);
            ...
            if (newVariant["available"]) {
              ...
              var addToCartContent = addToCartButtonElement.innerHTML;
              addToCartButtonElement.innerHTML = addToCartContent.replace('Sold out', window.languages.productFormAddToCart);
            } else {
              addToCartButtonElement.setAttribute("disabled", "disabled");
              addToCartButtonElement.classList.add("button--disabled");
              addToCartButtonElement.classList.remove("button--primary");
              addToCartButtonElement.removeAttribute("data-action");
              var addToCartContent = addToCartButtonElement.innerHTML;
              addToCartButtonElement.innerHTML = addToCartContent.replace('Add to cart', window.languages.productFormSoldOut);
            }
      ...
      },
      {
        ...
        value: function _onOptionChanged(event, target) {
          ...
          $(this.variantSelectors).find('.swatch_required_msg').removeClass('shake').addClass('hide');
          ...
          if (target.value != "NA") {
            ...
            let currentVariantTitle = this.currentVariant.title;
            localStorage.setItem('currentVariantTitle',currentVariantTitle);
            ...
            if (this.currentVariant) {
              ...
              // Frequently bought together main product info update
              $(target)
                .parents("body")
                .find(
                  ".frequently-product_detail-row.fbt-main .fbt_variants .variant_name"
                )
                .html(this.currentVariant.title);
              $(target)
                .parents("body")
                .find(".frequently-product_detail-row.fbt-main .fbt_price")
                .html(
                  Currency.formatMoney(
              ...
              $(target)
                .parents("body")
                .find(
                  '.frequently-product_detail-row.fbt-main input[name="id"]'
                )
                .val(this.currentVariant.id);
              $(target)
                .parents("body")
                .find(
                  '.frequently-product_detail-row.fbt-main input[name="fbt_check"]'
                )
                .data("price", this.currentVariant.price);

              //  updateFBTcurrentVariant(this,target);
      ...
      },
      {
        ...
        value: function _addToCart(event, target) {
          var _this4 = this;
          var cart_add = true;
          if(target.classList.contains("main_product_cart_btn")){
            localStorage.setItem("mainproductaddtocart", "showrelevant")
          }

          if($(_this4.variantSelectors).find('.block-swatch input[type="radio"]:checked').length == 0 && $(_this4.variantSelectors).find('.block-swatch input[type="radio"]').length != 0){
            if($(_this4.variantSelectors).closest('.quick-view-product-modal').length > 0){
              $('.quick-view-product-modal .modal__inner').animate({
                  scrollTop: $(".card__section .product-form").offset().top - 200
              }, 1000);
            }else{
              ...
              $('html, body').animate({
                scrollTop: $(".card__section .product-form").offset().top - topScrollValue
              }, 1000);
            }
            $(_this4.variantSelectors).find('.swatch_required_msg').removeClass('hide').addClass('shake');
            return false;
          }
          ...
          if (cart_add) {
            ...
            target.classList.add("btn-loader");
            ...
            fetch("".concat(window.routes.cartAddUrl, ".js"), {
              ...
              if (response.ok) {
                ...
                target.classList.remove("btn-loader");
      ...
      {
        key: "_updateWeight",
        value: function _updateWeight(newVariant, previousVariant) {
          /**
           * Update the Weight label
           */
          var productWeight = this.element.querySelector("#estimate_delivery");
          ...
          if (
            previousVariant &&
            previousVariant["weight"] === newVariant["weight"]
          ) {
            return;
          }

          productWeight.setAttribute('data-product-weight', newVariant["weight"]);
          if(document.getElementById('pin_code_to').value != ''){
  ...
  */
    ...
    _createClass(QuantityPicker, [
      ...
      },
      {
        ...
        value: function _onDecrease() {
          ...
          var qty = parseInt(this.inputElement.value);
          var productMobilePrice = $(this.element).closest('.product-form').find('.sticky-on-mobile .price--highlight');
          var quickMobilePrice = $(this.element).closest('.product-form').find('button.product-form__add-button .price--highlight');
          if(productMobilePrice){
            var productPrice = parseInt(productMobilePrice.attr('data-price'));
            productMobilePrice.html(Currency.formatMoney(productPrice * qty, window.theme.moneyFormat));
          }
          if(quickMobilePrice){
            var quickViewPrice = parseInt(quickMobilePrice.attr('data-price'));
            quickMobilePrice.html(Currency.formatMoney(quickViewPrice * qty, window.theme.moneyFormat));
          }
        },
      },
      {
      ...
      },
      {
        ...
        value: function _attachListeners() {
          ...
          this.flickityInstance = new js(
            this.element.querySelector(".upsell_products_slider"),
            {
              prevNextButtons: true,
              pageDots: false,
              cellAlign: "left",
  ...
  var LogoListSection = /*#__PURE__*/ (function () {
    function LogoListSection(element) {
      _classCallCheck(this, LogoListSection);

      this.element = element;
      this.flickityInstance = new js(this.element.querySelector(".logo-list"), {
        watchCSS: true,
        prevNextButtons: true,
          ...
          if (this.currentTags.includes(newTag)) {
            ...
            if (this.options["filterType"] === "tag") {
              ...
              this.currentTags.forEach(function (existingTag) {
                // AE-Apr-028 Filter & Tags on Collection Page
                //if (existingTag.split('_')[0] === tagGroup) {
                if (existingTag === tagGroup) {
    ...
    _createClass(FeaturedCollectionSection, [
      ...
      },
      {
        ...
        value: function _addToCart(event, target) {
          ...
          var selectedElementValue = target.closest('form[action*="/cart/add"]').querySelector('.product-form__single-selector');
          if (selectedElementValue == 'NA' || selectedElementValue == 'null' || selectedElementValue == 'undefined') {
            return false;
          }
  ...
  var ProductAdditionalWidgets = /*#__PURE__*/ (function () {
    function ProductAdditionalWidgets(element) {
      _classCallCheck(this, ProductAdditionalWidgets);
      ...
      this.productItemColorSwatch = new ProductItemColorSwatch(this.element);
      if(this.options['ProductMeta'] || this.options['MetaName'] == 'brand'){
        this._getSearchQueryString();
      }else{
        if (!this.options["stackable"]) {
          this.flickityInstance = new js(
            this.element.querySelector(".product-list"),
    ...
    _createClass(ProductAdditionalWidgets, [
      {
        key: "onUnload",
        value: function onUnload() {
          if (!this.options["stackable"]) {
      ...
      },
      {
        ...
        value: function _fetchProducts(items) {
          ...
          let MetaName = this.options['MetaName'];
          if (queryString === "") {return}
          let template = "pdp-additional-widgets";
          fetch("".concat(window.routes.searchUrl, "?view=").concat(template, "&type=product&q=").concat(queryString),{
            credentials: "same-origin",
            method: "GET",
          }).then(function (response) {response.text().then(function (content) {
              let tempElement = document.createElement("div");
              tempElement.innerHTML = content; // Set the content

              _this.element.querySelector(`.product_${MetaName}_widget .product-related`).innerHTML = tempElement.querySelector('.pdp_additional_widgets').innerHTML;
              ...
              if (window.ResizeObserver && _this.flickityInstance) {
                ...
                _this.element.querySelectorAll(".product-item").forEach(function (item) {_this.resizeObserver.observe(item)});
              }
      ...
      },
      {
        ...
        value: function _getSearchQueryString() {
          let items = [],baseURL = theme.stApiUrl,baseAuthorization = `Bearer ${theme.stApiAuthorization}`,CurrentProductId = this.options['productId'];
          let dataPayload = this._getStDataPayload();
          fetch(baseURL,{body: JSON.stringify(dataPayload),credentials: "same-origin",method: "POST",
            headers: {"Content-Type": "application/json","authorization": baseAuthorization},
          }).then((response) => response.json()).then((data) => {
            data.results.forEach(function(e){if(CurrentProductId != e.id) items.push(e.id)});
            let newitems = items.map(function (item) {return "id:" + item;}).join(" OR ");
            this._fetchProducts(newitems);
            this._attachListeners();
          });
        },
      },
      {
        key: "_getStDataPayload",
        value: function _getStDataPayload() {
          let MetaName = this.options['MetaName'],ProductCol = this.options['ProductCol'], ProductColHandle = this.options['ProductColHandle'],ProductMeta = this.options['ProductMeta'],
          finalProductMeta = [],textFacetFilters = {};
          if(MetaName == 'material'){
            if(ProductMeta.includes(',')){
              let splitMeta = ProductMeta.split(',');
              for (let m = 0; m < splitMeta.length; m++) finalProductMeta.push(splitMeta[m]);
            }else{
              finalProductMeta.push(ProductMeta);
            }
            textFacetFilters.meta_material = finalProductMeta;
          }else if(MetaName == 'type'){
            finalProductMeta.push(ProductMeta);
            textFacetFilters.product_type = finalProductMeta;
          }else if(MetaName == 'brand'){
            if(ProductMeta != ''){
              finalProductMeta.push(ProductMeta);
              textFacetFilters.collections = finalProductMeta;
            }
          }
          let dataPayload = {
            "query": "","fields": ["handle","id"],"textFacets": ["product_type","meta_material"],"searchFields": ["*"],
            "filter": `isSearchable = 1 AND collection_handles_shopify='${ProductColHandle}'`,
            "sort": ["-isActive",`${ProductCol}_position`],"count": 24,
            "collection": theme.stApiCollection,"facetCount": 499,"groupCount": -1,"typoTolerance": 1,
    ...
    return ProductAdditionalWidgets;
  })();

  var pdpCollectionBanners = /*#__PURE__*/ (function () {
    function pdpCollectionBanners(element) {
      _classCallCheck(this, pdpCollectionBanners);
      ...
      this._fetchProducts(this.options['products_json']);
      this._attachListeners();
    }

    _createClass(pdpCollectionBanners, [
      {
        key: "onUnload",
        value: function onUnload() {
          if (!this.options["stackable"]) {
        ...
        value: function _fetchProducts(items) {
          ...
          fetch("".concat(window.routes.searchUrl, "?view=").concat(template, "&type=product&q=").concat(queryString),{
            ...
            method: "GET",
              ...
              _this.element.querySelector('.collection_banners_products_slider .product-related').innerHTML = tempElement.querySelector('.pdp_additional_widgets').innerHTML;
    ...
    return pdpCollectionBanners;
  })();

  var pdpCollectionMetafieldBanners = /*#__PURE__*/ (function () {
    function pdpCollectionMetafieldBanners(element) {
      _classCallCheck(this, pdpCollectionMetafieldBanners);
    ...
    _createClass(pdpCollectionMetafieldBanners, [
      {
        key: "onUnload",
        value: function onUnload() {
          if (!this.options["stackable"]) {
    ...
    return pdpCollectionMetafieldBanners;
  })();
      ...
      },
      {
        ...
        value: function toggleMobileSearch() {
          if (this.isSearchOpen) {
            ...
            document.body.classList.remove('body-search-expanded');
            this.element.classList.remove("is-visible");
            
          } else {
            this.headerElement.classList.add("header--search-expanded");
            document.body.classList.add('body-search-expanded');
            this.element.classList.add("is-visible");
          }
  ...
  var LoginSection = /*#__PURE__*/ (function () {
    function LoginSection(element) {
      ...
      this.element = element;
      this.domDelegate = new Delegate(this.element);
      this.customerLoginForm = this.element.querySelector("#login_register_step");
  ...
  var youMayAlsoLikeProductsSection = /*#__PURE__*/ (function () {
    function youMayAlsoLikeProductsSection(element) {
      _classCallCheck(this, youMayAlsoLikeProductsSection);
    ...
    _createClass(youMayAlsoLikeProductsSection, [
      {
        key: "onUnload",
        value: function onUnload() {
          if (this.flickityInstance) {
      ...
      },
      {
        ...
        value: function _fetchProducts(items) {
          ...
          var template = "auto-related-products";
          fetch(
            ""
              .concat(window.routes.searchUrl, "?view=")
              .concat(template, "&type=product&q=")
            ...
            response.text().then(function (content) {
              ...
              tempElement.innerHTML = content; // Set the content
              _this.element.querySelector(
                ".product-related-you_may_like"
              ).innerHTML = tempElement.querySelector(
                '[data-section-type="product-recommendation-you-may-like"] .product-related-you_may_like'
              ).innerHTML; // By default the section hide the products, so we force the section to be visible
      ...
      },
      {
        ...
        value: function _getSearchQueryString() {
          var items = [];
          let currentProductids = this.options["productId"];

          fetch(`${theme.apiUrl}/youMayAlsoLike.php?product_id=${currentProductids}`)
            .then((response) => response.json())
            .then((data) => {
              data.products.forEach(function(e){
                items.push(e.id);
              })
              var newitems = items.map(function (item) {return "id:" + item;}).join(" OR ");
                this._fetchProducts(newitems);
                this._attachListeners();
            });
        },
    ...
    return youMayAlsoLikeProductsSection;
  })();

  var viewsimilarProductsSection = /*#__PURE__*/ (function () {
    function viewsimilarProductsSection(element) {
      _classCallCheck(this, viewsimilarProductsSection);
    ...
    _createClass(viewsimilarProductsSection, [
      {
        key: "onUnload",
        value: function onUnload() {
          if (this.flickityInstance) {
      ...
      },
      {
        ...
        value: function _fetchProducts(items) {
          ...
          let template = "";
          if (!_this.options["similarPopup"]) {
            template = "view-similar";
          }else{
            template = "view-similar-popup";
          }
          fetch("".concat(window.routes.searchUrl, "?view=").concat(template, "&type=product&q=").concat(queryString),{
            credentials: "same-origin",
            method: "GET",
            ...
            response.text().then(function (content) {
              var tempElement = document.createElement("div");
              tempElement.innerHTML = content; // Set the content
              _this.element.querySelector(".product-related").innerHTML = tempElement.querySelector('.pdp_viewsimilar_widgets').innerHTML;
              ...
              if (!_this.options["similarPopup"]) {
                _this.element .querySelectorAll(".product-item") .forEach(function (item) {
                  if (_this.options["enableImageZoom"]) {
                    item.querySelector(".button_zoom_quicklook").addEventListener("click", _this._openMobileZoom.bind(item));
                  }
                });
                if (_this.options["enableImageZoom"]) {
                  $(document).on('click','.product_view_similar_pswp .pswp__button',function(){
                    let parent = document.querySelector(".product-item.active_zoom");
                    _this._doPswpAction(parent,this);
                  });
                }
              } 
      ...
      },
      {
        ...
        value: function _getSearchQueryString() {
          var items = [];
          let currentProductids = this.options["productId"];
          fetch(`${theme.apiUrl}/versions/v1/pdp/similar-products?productID=${currentProductids}`)
            .then((response) => response.json())
            .then((data) => {
              if(data.status == false){
                if (_this.options["similarPopup"]) {
                  ...
                  _this.element.querySelector(".view-similar-product-wrapper__inner").innerHTML = NofoundHtml;
                }
              }else{
                data.products.forEach(function(e){
                  items.push(e.id);
      ...
      },
      {
        ...
        value: function _openMobileZoom() {
          let _this4 = this;
          _this4.classList.add("active_zoom");
          $('#shopify-section-header').css('z-index','0');
          // On mobile, we use instead PhotoSwipe
          let pswpElement = document.querySelector(".product_view_similar_pswp");

          let filteredItems = this.querySelectorAll(
              '.product-gallery__carousel-item'
            ),
            defaultIndex = 0,
            items = [];
          ...
          this.photoSwipeInstance.listen("beforeChange", function () {
            let currentItem = pswpElement.querySelector(
                ".pswp__pagination-current"
              ),
              paginationCount = pswpElement.querySelector(
                ".pswp__pagination-count"
              );
      ...
      },
      {
        ...
        value: function _doPswpAction(event, element) {
          let __this = event;
          if (!__this.photoSwipeInstance) {return;}

          if (element.classList.contains("pswp__button--close")) {
            __this.photoSwipeInstance.close();
            __this.classList.remove("active_zoom");
            $('#shopify-section-header').css('z-index','8');
          } else if (element.classList.contains("pswp__button--arrow--left")) {
            __this.photoSwipeInstance.prev();
          } else if (element.classList.contains("pswp__button--arrow--right")) {
            __this.photoSwipeInstance.next();
          }
        },
      },
      {
        key: "_handleZoomForMediaQuery",
        value: function _handleZoomForMediaQuery(event) {
          let __this = event;
          __this.photoSwipeInstance.close();
        },
      },
    ...
    return viewsimilarProductsSection;
  })();
  ...
  var VideoSliderSection = /*#__PURE__*/ (function () {
    function VideoSliderSection(element) {
      var _this = this;
      this.element = element;
      this._loadYouTubeScript();
      $(document).on('click','.video-slider-box .video__carousel-item',function(e){
        e.preventDefault();
        _this._setupYouTubePlayer(this);
      });
    }
    _createClass(VideoSliderSection, [
      {
        key: "_setupYouTubePlayer",
        value: function _setupYouTubePlayer(elem) {
          var _this2 = elem,
              _model = document.querySelector('.video_popup_model'),
              _modelOverlay = document.querySelector('.video_popup_model_overlay');
          _this2.player = new YT.Player(
            _model.querySelector(".iframe"),
            {
              videoId: _this2.getAttribute("data-video-id"),
          ...
          _model.classList.add('active');
          _modelOverlay.classList.add('active');
        },
      },
      {
        key: "_loadYouTubeScript",
    ...
    return VideoSliderSection;
  })();

  // HomeCommunity
  var HomeCommunity = /*#__PURE__*/ (function () {
    function HomeCommunity(element) {
      _classCallCheck(this, HomeCommunity);
      this.element = element;
      this.delegateElement = new Delegate(this.element);
      let maxHeight = Math.max.apply(null, $(".home_community_silder .community_block").map(function (){
        return $(this).find('h3').height();
      }).get());
      $(".home_community_silder .community_block h3").height(maxHeight);
      if(this.element.querySelector(".home_community_grid") == null){
        this.flickityInstance = new js(
          this.element.querySelector(".home_community_silder"),
          {
            watchCSS: true,
            pageDots: false,
            prevNextButtons: true,
  ...
  // conscious-journey
  var ConsciousJourneySection = /*#__PURE__*/ (function () {
    function ConsciousJourneySection(element) {
      _classCallCheck(this, ConsciousJourneySection);
      this.element = element;
      this.delegateElement = new Delegate(this.element);

      this.flickityInstance = new js(
        this.element.querySelector(".home_conscious_journey_silder"),
        {
          watchCSS: true,
          pageDots: false,
          prevNextButtons: true,
    ...
    _createClass(ConsciousJourneySection, [
      {
        key: "onUnload",
        value: function onUnload() {
          this.delegateElement.off("change");
    ...
    return ConsciousJourneySection;
  })();

  // Home customer review
  var CustomerReviewSection = /*#__PURE__*/ (function () {
    function CustomerReviewSection(element) {
      _classCallCheck(this, CustomerReviewSection);
      this.element = element;
      this.delegateElement = new Delegate(this.element);

      this.flickityInstance = new js(
        this.element.querySelector(".home_customer_review_silder"),
        {
          watchCSS: true,
          pageDots: false,
          prevNextButtons: true,
    ...
    _createClass(CustomerReviewSection, [
      {
        key: "onUnload",
        value: function onUnload() {
          this.delegateElement.off("change");
  ...
  //Home Featured In
  var HomeFeaturedIn = (function () {
    function HomeFeaturedIn(element) {
      _classCallCheck(this, HomeFeaturedIn);
      this.element = element;
      this.delegateElement = new Delegate(this.element);

      this.flickityInstance = new js(
        this.element.querySelector(".home_featured_list_silder"),
        {
          watchCSS: true,
          pageDots: false,
          prevNextButtons: true,
    ...
    return HomeFeaturedIn;
  })();

  var CollectionCarouselSection = (function () {
    function CollectionCarouselSection(element) {

      _classCallCheck(this, CollectionCarouselSection);
      this.element = element;
      this.delegateElement = new Delegate(this.element);
      this.options = JSON.parse(
        this.element.getAttribute("data-section-settings")
      ...
      this.flickityInstance = new flickityFade(
        element.querySelector(".collection-banners_slider"),
        {
          watchCSS: true,
          pageDots: this.options["pageDots"],
          prevNextButtons: true,
    ...
    _createClass(CollectionCarouselSection, [
      {
        key: "onUnload",
        value: function onUnload() {
          this.delegateElement.off("change");
  ...
  (function () {
    var bootTheme = function bootTheme() {
      ...
      sections.register("logo-list", LogoListSection);
      ...
      sections.register(
        "product-recommendation-you-may-like",
        youMayAlsoLikeProductsSection
      );
      sections.register("product-viewsimilar-section",viewsimilarProductsSection);
      ...
      sections.register("video_slider_section", VideoSliderSection);
      sections.register("app-popup", AppPopupSection);
      sections.register("home-community", HomeCommunity);
      sections.register("conscious-journey", ConsciousJourneySection);
      sections.register("collection-banners-carousel", CollectionCarouselSection);
      sections.register("home-customer-review", CustomerReviewSection);
      sections.register("home-featured-in-list", HomeFeaturedIn);
      sections.register("product-additional_widgets", ProductAdditionalWidgets);
      sections.register("collection_banners-products", pdpCollectionBanners);
      sections.register("pdp_collection_banners_products", pdpCollectionMetafieldBanners);
      /**
       * ----------------------------------------------------------------------------
       * RTE
       * ----------------------------------------------------------------------------
    ...
    // th_product_bundle QuickView
    setTimeout(function(){
      const th_product_bundle = $('#th_product_bundle').html();
      if(th_product_bundle != '') {
        $('.style_the_look_block .th_pb_section .th_product_img a').on("click",function(event){
          event.preventDefault();
          const _this = $(this);
          thOpenQuickView(event,_this);
        });
        const modalClosebtn = $('#modal-quick-view-product-template_view_similar [data-action="close-modal"]');
        modalClosebtn.on('click',function(){$('#modal-quick-view-product-template_view_similar').attr("aria-hidden", "true")});
        const modalCloseClass = $('#modal-quick-view-product-template_view_similar');
        modalCloseClass.on('click',function(event){thclickOutsideListener(event,modalCloseClass)});
      }
    },5000);
    function thOpenQuickView(event,target) {
      window.history.pushState("forward", null, " ");
      let modal = document.querySelector('#modal-quick-view-product-template_view_similar');
      modal.classList.add("is-loading");
      modal.setAttribute("aria-hidden", "false");
      fetch("".concat(
          target[0].href,
        ...
        response.text().then(function (content) {
          modal.querySelector(".modal__inner").innerHTML = content;
          modal.classList.remove("is-loading"); // Register a new section to power the JS
          let modalProductSection = new ProductSection(
            modal.querySelector('[data-section-type="product"]')
          ); // We set a listener so we can cleanup on close
          let doCleanUp = function doCleanUp() {
            modalProductSection.onUnload();
            modal.removeEventListener("modal:closed", doCleanUp);
          };
    ...
    function thclickOutsideListener(event,modal){
      let container = $("#modal-quick-view-product-template_view_similar .modal__inner");
...
// Product page review changes

$(document).ready(function () {
  setTimeout(function () {
    $('#stamped-sort-select option[value="featured"]').html("Featured");
    if ($(".stamped-review").length > 2) {
      var reviews_list = $(".stamped-review");
      var total_reviews = $(".stamped-review").length;
      for (var i = 2; i <= total_reviews; i++) {
        $(reviews_list[i]).hide();
      }
      $(".stamped-pagination").hide();

      $(".stamped-reviews").append(
        '<div class="reviews_see"><span class="reviews_see_more button cutom-button">See all reviews</span></div>'
      );
      //$('.reviews_see_less').hide();
      $(".reviews_see_more").click(function () {
        for (var i = 2; i <= total_reviews; i++) {
          $(reviews_list[i]).toggle();
        }
        $(".reviews_see_more").hide();
        //$('.reviews_see_less').toggle();
        $(".stamped-pagination").toggle();
      });
    }
  }, 2000);
});
...
function gstValidation($this) {
  ...
  if(inputvalues == ''){
    ...
    parent.find(".form-error").text('Please enter a valid GSTIN').addClass("active"); 
    $this.addClass('error');
    count++;
  } 
  return count;
...
$(document).on('keypress','.txtAlphaNumeric', function (e) {
    var k = e.which || e.keycode;
    $(this).removeClass('error');
    $(this).closest(".form-wrap").find(".form-error").removeClass("active"); 
    return ((k >= 48 && k <=57) || (k >= 65 && k <=90) || (k >= 97 && k <=122));
});
$(document).on('submit','[action="/cart"]', function(e){
  var error_count = 0;
  error_count = error_count + gstValidation($(this).find('[name="attributes[CustomerGSTIN]"]'));
  if (error_count > 0) {
    return false;
  }
});
...
//     var nav = $('.cart-recap');
//     if (nav.length) {
//        var div_top = $('.cart-recap').offset().top - 200;
//       if (window_top > div_top) {
//         $('body').addClass('stick-feature');
//       } else {
//         $('body').removeClass('stick-feature');
//       }
//     }
...
//fixed slider on left

//   $(function() {
//     var nav2 = $('.cart-recap');
//     if (nav2.length) {
//       var top = $('.cart-recap').offset().top - 190;
//       var footTop = $('#shopify-section-about-text-icons').offset().top - 190;

//       var maxY = footTop - $('.cart-recap').outerHeight();
...
//           if (y < maxY) {
//             $('.cart-recap').addClass('fixed').removeAttr('style');
//           } else {

//             //Quand la sidebar arrive au footer, supprime la classe "fixed" précèdement ajouté
//             $('.cart-recap').removeClass('fixed').css({
//               position: 'absolute',
//                top: (maxY - top) + 'px'
//             });
//           }
//         } else {
//           $('.cart-recap').removeClass('fixed');
//         }
//       });
//     }
...
$(document).on('click','.form__submit.custom_loading',function(e){
  $(this).addClass('loading');
});
$(document).on('keyup input','.header .search-bar .search-bar__input',function(){
  if(!$(this).closest('.header').hasClass('header--search-expanded') && $(this).val().length > 0 && $('.header:not(.header--search-expanded) [data-action="toggle-search"]').length > 0){
    $('.header:not(.header--search-expanded) [data-action="toggle-search"]')[0].click();
  }
});

$(document).on('click','#account-popover a.popover__link-item[data-no-instant], #logout',function(e){
  localStorage.removeItem("themeDisBanner");
  localStorage.removeItem('themeUserNew');
  localStorage.removeItem('themeDisBannerShow');
  localStorage.removeItem('cartDisBanner');
});
...
$(document).on('click','.newsletter_subscription_button',function(e){
  e.preventDefault();
  let _this = $(this),
  ...
  if(email == ''){
    $form.find('input[name="contact[email]"]').siblings('.form_error').text('Email is required.').show();
    return false;
  }else{$form.find('input[name="contact[email]"]').siblings('.form_error').hide()}
  if(phone != '' && phone.length != 10){
    $form.find('input[name="contact[phone]"]').siblings('.form_error').text('Enter valid phone number.').show();
    return false;
  }else{$form.find('input[name="contact[phone]"]').siblings('.form_error').hide()}
  ...
  $.ajax({
    type: "POST",
    url: theme.apiUrl+"/subscriptionPopup.php",
    data: formObject,
    dataType: "json",
    success: function (data) {
      _this.removeClass('loading');
      if(data.error_phone == false){
        $form.find('input[name="contact[phone]"]').siblings('.form_error').text(data.message).show();
      }else{
        if(data.customer == true && data.orders_count == 0){
          $form.find('> *').hide();
          $form.find('.newsletter_form_posted_successfully').removeClass('hide').show();
        }else if(data.customer == true){
          $form.find('> *').hide();
          $form.find('.newsletter_form_posted_successfully').removeClass('hide').show();
          $form.find('.newsletter_form_posted_successfully .popup-newsletter__content').hide();
        }else{
          $form.find('> *').hide();
          $form.find('.newsletter_form_posted_successfully').removeClass('hide').show();
        }
...
function handleCredentialResponse(response) {
  ...
  $.ajax({
    type: "POST",
    url: theme.apiUrl+"/versions/v1/user/login",
    data: responseObject,
    dataType: "json",
    success: function (response) {
      const return_to = location.pathname;
      window.location.href = 'https://amala.earth/account/login/multipass/'+response.token+'/?return_to='+return_to;
      if(response.customer == false){localStorage.setItem('themeUserNew',true);}
    },
    error: function (error) {
      console.log('error',error);
    }
...
let cancellationProcess = (event,_this) => {
    event.preventDefault();
    let $form = _this.closest('form'),
    reason = $form.find('select[name="reason"]').val(),
    orderId = $form.find('input[name="o_id"]').val(),
    customerId = $form.find('input[name="c_id"]').val();
    if(reason == 'Other'){
        let reasonOther = $('.for_other_reason textarea[name="comment"]').val();
        reason = reasonOther;
    }
    let formObject = {
        "order_id": orderId,
    ...
    _this.addClass('loading');
    $.ajax({
        url: theme.apiUrl+'/versions/v1/order',
        type: 'POST',
        data: formObject,
        success: function(response, textStatus, xhr) {
            if(textStatus == 'success' && xhr.status == 200){
              _this.removeClass('loading');
              $('#cancle-order-model').removeClass('show-pop-up');
              $('#successful-cancel-message').addClass('show-pop-up');
              $('.cancellation-cta').addClass('my-orders__item-actions-link-disabled');
              $('.my-orders__item-actions-invoice').addClass('my-orders__item-actions-link-disabled');
              $('.status-Confirmed').text('Cancelled');
              $('.my-orders__track-list').html(`<li class="my-orders__track-list-item"><label>Cancelled - ${formatAMPM(new Date)}. </label></li>`);
              $('.est_deliver_date_wrap').hide();
            }else{
...
let formatAMPM = (date) =>  {
    ...
    currentDate = date.getDate(),years = date.getFullYear(),month = monthNames[date.getMonth()];
    let hours = date.getHours(),minutes = date.getMinutes(),
    ampm = hours >= 12 ? 'pm' : 'am';
    hours = hours % 12;
    hours = hours ? hours : 12;
    minutes = minutes < 10 ? '0'+minutes : minutes;
    let strTime = hours + ':' + minutes + ' ' + ampm,fulldate = `${month} ${currentDate}, ${years} at ${strTime}`;
    return fulldate;
}
// $(document).on('click', '#cancel_form button[type="submit"]', function(event) {
//   event.preventDefault();
//   cancellationProcess(event,$(this));
// });
// $(document).on('click', '#successful-cancel-message .btn-ok', function(event) {
//     event.preventDefault();
//     $('#successful-cancel-message').removeClass('show-pop-up');
// });
// $(document).on('change', '.my-account__pop-up_form-wrapper select[name="reason"]', function(event) {
//     event.preventDefault();
//     let value = $(this).val();
//     if(value == 'Other'){
//         $('.for_other_reason').removeClass('hide');
//     }else{
